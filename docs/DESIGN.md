...
- **Los niveles NUNCA bajan como castigo.** Ni por inactividad ni por "recaídas". Esta es una decisión de diseño explícita y central.
- **Excepción: deshacer un registro de hoy.** La regla anterior protege contra el castigo — algo que el usuario hizo (o dejó de hacer) y que el sistema le reprocha. Corregir un registro equivocado del mismo día (foco o intensidad mal elegidos) es distinto: esa XP nunca se ganó de verdad, así que restarla no es un castigo, es arreglar un dato mal introducido. **Implementado**: `DELETE /activities/:id`, acotado a actividades de hoy — la ventana existe para que sea corrección de un despiste reciente y no un mecanismo para "arrepentirse" de un día entero con perspectiva. Revierte la XP en cascada (foco si lo tenía, y categoría) y recalcula el nivel de verdad, sin el tope de "nunca baja" que sí aplica a `applyXp`. Si el foco estaba congelado y cae por debajo de nivel 20, se descongela.
...
- **Nivel como insignia de rango** (`client/src/components/LevelNumber.tsx`, clases `level-*` en `index.css`): la cifra en hueso con trazo negro y **extrusión maciza** (una pila de sombras negras, rotulación de cómic), sobre una **estrella de puntas amarilla** con filo negro y una cinta "NV" clavada en la esquina. La estrella **gana una punta cada 4 niveles** (7 en el nivel 1, tope 22): la silueta dice sin leer lo avanzado que vas. La forma se deriva del nivel, nunca al azar, y sus puntas son irregulares como los rayos del fondo. Se balancea despacio y de forma continua, con cada insignia desfasada; al subir de nivel entra girando. En las cartas de categoría y fichas de foco va **apoyada sobre la banda**, no colgando por fuera. Antes se probaron letras de nota de rescate y una cifra lisa; se descartaron
- **Cifras grandes en Archivo Black** (`--font-figure`: niveles, el día de la fecha, la racha), cargando solo sus diez dígitos. Dela Gothic One, con un trazo grueso, cierra la abertura del 5 hasta parecer un 6 y la del 3 hasta parecer un 8. La racha va en una placa negra con sombra amarilla para no perderse entre los rayos
- Rótulos (`text-sign`): trazo negro + una sola sombra negra dura. La sombra mostaza anterior se leía como suciedad sobre bandas de color
- Utilidades y piezas (definidas en `client/src/index.css`):
  ...
  - **Fondo: el estallido** (`scene-burst` en `client/src/index.css`, capas en `App.tsx`). Rayos duros de anchos desiguales que salen de detrás de la cabecera — el logo en la home, la marca de la categoría en su detalle — y giran de forma continua (`linear`, 70 s por vuelta): con `steps()` avanzaban a saltos y parecía que el fondo se atascaba — es la única excepción a los cortes secos de la app. Toman el color de `--light` (`useLight`): rojo en la home, el de la categoría dentro de ella. Llegan hasta el borde inferior de la pantalla, pero hacia abajo pasan a **trama en bandas duras** (puntos negros que crecen y, al final, los rayos solo a través de agujeros). Antes acababan en negro liso al 70% de la pantalla y parecía que el fondo se cortaba. Encima, las estrellas del logo parpadeando y grano de película. Sustituye a la "sala de interrogatorio" (haz de luz tenue), que no llegaba a verse
  - Los rayos giran animando el ángulo del degradado (`@property --spin`), no rotando la capa: rotarla exigiría una textura del tamaño de la diagonal de la pantalla. La animación se llama `ray-spin` porque Tailwind ya trae un `spin` que la pisaba
  - El modal de resultado usa el mismo estallido a todo color, con el contenido sobre un panel negro propio
  - Las texturas siguen siempre **detrás** del contenido: superpuestas actúan como veladura y apagan el color de las tarjetas
...
- Layout:
  ...
  - **Gesto de los Focos, igual en la home y en el detalle** (`useQuickLog` + `QuickIntensities`): tocar un foco activo despliega las tres intensidades y registra en el sitio; tocar uno congelado engendra su hijo. Mantener pulsado, o el asa de tres rombos del borde derecho, abre la hoja de gestión (`FocusMenu`: renombrar, cerrar/reabrir, borrar). El asa existe porque la pulsación larga no se descubre mirando ni se alcanza con teclado
