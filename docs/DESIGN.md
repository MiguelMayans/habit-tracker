...
- **Nivel con la misma rotulación que el día de la fecha de la home** (`client/src/components/LevelNumber.tsx`): Dela Gothic One en hueso, trazo negro grueso y una sombra negra dura (`text-sign`; `text-sign-fine` por debajo de 30px). Se probaron letras de nota de rescate (cada cifra en su recorte de papel, con tipografía y giro propios) y se descartaron: la cifra lisa y pesada se lee mejor de un vistazo. En el detalle de categoría va sobre placa negra, no hueso
