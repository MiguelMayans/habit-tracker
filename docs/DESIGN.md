...
- **Nivel** (`client/src/components/LevelNumber.tsx`, clases `level-*` en `index.css`): la cifra en hueso con trazo negro y **extrusión maciza** (una pila de sombras negras, rotulación de cómic), con una cinta "NV" a su izquierda, a la altura de su parte alta, en lugar de una etiqueta "NIVEL" suelta. Al subir de nivel entra de golpe. En las cartas de categoría y fichas de foco va **apoyada sobre la banda, a la derecha**, no colgando por fuera. Se probaron y descartaron: letras de nota de rescate, una cifra lisa y una estrella de puntas detrás (demasiado)
