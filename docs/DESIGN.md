...
- **Nivel en letras de nota de rescate** (`client/src/components/LevelNumber.tsx`): cada cifra en su propio recorte de papel, con su tipografía, color y giro — la rotulación de las tarjetas de aviso, y el mismo lenguaje que las marcas de categoría. Se deriva de la cifra y su posición (nunca al azar), así que un número siempre se ve igual. Para los recortes se cargan **Anton** y **Abril Fatface**, solo sus diez cifras (`text=` de Google Fonts). Es tipografía y no imagen: una lámina de cifras dibujadas se probó y se descartó. Solo para niveles de 24px en adelante; los niveles de las filas pequeñas de la home siguen en `font-display`
